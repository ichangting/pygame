...
class Point(object):
    def __init__(self, x, y):  #__init__()，以双下划线开头和结尾代表类的构造函数 
        self.__x = x  # __x,以双下划线开头代表类的私有成员
        self.__y = y
...
width,height = superman.get_size() #获取位图的宽和高
superman = pygame.transform.smoothscale(superman,(width//2,height//2))  # 对图像进行缩放
